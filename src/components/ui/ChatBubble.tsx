...
const ChatBubble = ({
  ...
  const isOwner = align === "end" || role === "ADMIN" || role === "MODERATOR"
  ...
  return (
    <div
      className={cn("flex gap-3 mb-4", isStart ? "flex-row" : "flex-row-reverse")}
    ...
    >
      ...
      <div className={cn("flex flex-col max-w-[80%] relative", isStart ? "items-start" : "items-end")}>
        {/* Header */}
        ...
        <div
          className={cn(
            ...
            isStart
              ...
              : "bg-teal-500 text-white text-right rounded-br-none"
          )}
        >
          {isEditing ? (
            <div>
              <input
                type="text"
                className="bg-transparent border-b outline-none w-full text-black dark:text-white"
                value={editedContent}
          ...
          {isOwner && hover && !isEditing && (
            <div className={`absolute -top-10 right-0 left-0`}>
              ...
              <button
                onClick={() => onDelete(id)}
                className="text-xs text-neutral-400 hover:underline hover:text-teal-800 duration-300"
                title="Delete"
              >
                <Trash2 size={14} />
              </button>
